...
def build():
    ...
    add_table(doc, ["项目", "内容"], [
        ...
        ["版本", "V1.3"],
        ["编写日期", "2026-06-22"],
        ["适用对象", "项目组、指导教师、只读观摩用户"],
    ])

    add_title(doc, "1. 引言", 1)
    add_title(doc, "1.1 编写目的", 2)
    add_para(doc, "本文档是新能源充电桩大数据分析项目的顶层需求规格说明，规定系统应具备的功能、性能、数据、接口与安全需求及完成标准，作为概要设计、详细设计、编码实现、测试与项目交付的正式依据。")
    ...
    add_table(doc, ["项", "内容"], [
        ...
        ["完成标准", "hdfs dfs -ls /Car 显示4文件；可head查看"],
    ])
    ...
    add_title(doc, "7. 完成标准总表", 1)
    add_table(doc, ["模块", "最低标准", "项目目标"], [
        ["HDFS", "4文件可访问", "完成"],
        ["MapReduce", "≥3任务", "7任务"],
        ["MySQL", "≥3表有数据", "7表"],
