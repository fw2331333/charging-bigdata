...
def _eval_regression(model, x_test: np.ndarray, y_test: np.ndarray) -> dict[str, float]:
    """在测试集上计算回归评估指标。"""
...
def train_fee_with_comparison(rows: list[dict]) -> tuple[dict, dict]:
    ...
    lr_metrics = _eval_regression(lr_model, x_test, y_test)

    # 候选 2：XGBoost（指导文档推荐，适合非线性特征交互）
    ...
    xgb_metrics = _eval_regression(xgb_model, x_test, y_test)
