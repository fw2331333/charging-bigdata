...
function Get-DatartHeaders {
    $r = Invoke-WebRequest -Uri "$DatartUrl/api/v1/users/login" -Method Post -ContentType "application/json" -Body '{"username":"admin","password":"123456"}' -UseBasicParsing
...
$viewDefs = @(
    ...
    @{ name = 'v2_cell_voltage_range'; script = "SELECT 'maxcv' AS cell_type, SUM(max_cell_voltage) AS cell_voltage FROM t_cell_voltage_range UNION ALL SELECT 'mincv', SUM(min_cell_voltage) FROM t_cell_voltage_range" },
    @{ name = 'v3_temperature'; script = 'SELECT SUBSTRING(record_time, 9, 2) AS hour_bucket, SUM(max_temperature) AS max_temperature FROM t_temperature GROUP BY SUBSTRING(record_time, 9, 2) ORDER BY hour_bucket' },
...
Write-Host "Login:    $DatartUrl  (demo / 123456)"
...
if (Test-Path $buildScript) {
    ...
    & $buildScript -DatartUrl $DatartUrl -OrgId $OrgId -DashboardId $dashboardId -DashboardName $DashboardName
}
